...
const Testimonial = () => {
  ...
  return (
    <div className='flex flex-col gap-5 md:gap-10 items-center max-md:pb-10 py-12 mt-5 md:mt-20 md:mb-20 max-md:px-4'>
      ...
      <div className='flex gap-8 overflow-x-scroll w-full no-scrollbar scroll-smooth'>
      ...
      <div className='flex gap-2 md:hidden max-md:mb-6'>
        {reviews.map((item, index) => (
          <a key={item.name} href={`#review-${index}`} onClick={() => setActiveIndex(index)}><div className={`rounded-full shadow w-2 h-2 border border-brightred ${index === activeIndex && 'bg-brightred'}`}></div></a>
        ))}
      </div>
