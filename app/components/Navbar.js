...
const Navbar = () => {
  ...
  return (
    <div role='header' className='spacing relative overflow-hidden'>
      ...
      <nav role='navigation' className="flex justify-between items-center pb-5 pt-10 md:hidden">
        <Logo color="#242D52"/>
        <div onClick={() => sidebarBtnClick()}>
          {sidebarStatus ? 
            <Image priority={false} src="/images/icon-close.svg" alt='hamburger' width={25} height={18}/>
          :
            <Image priority={false} src="/images/icon-hamburger.svg" alt='hamburger' width={25} height={18}/>
          }
        </div>
      </nav>
      ...
      <Image priority={false} src="/images/bg-tablet-pattern.svg" alt='tablet-pattern' className='absolute bottom-0 right-[-210px] md:hidden z-[-1]' width="814" height="814" />
    </div>
  )
}
